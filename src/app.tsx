...
export default function Chat() {
  ...
  const agent = useAgent({
    agent: "chat",
    // host: "https://yirifi-ai-agent.yirifi-ai.workers.dev",
    host: "http://localhost:8787",
    // get this from url path
    name: (() => {
  ...
  const toolResponse = agentMessages
    .map((m: Message) => {
      let data = m.parts?.find(
        (part) =>
          part.type === "tool-invocation" &&
          part.toolInvocation.state === "result" &&
          part.toolInvocation.toolName === "getNeo4jData"
      );
      return data;
    })
    .filter(Boolean);
  // console.log(neo4jresponse[0]);
  //@ts-ignore
  const neo4jresponse = toolResponse[0]?.toolInvocation?.result;
                      ...
                      <div>
                        {m.parts?.map((part, i) => {
                          ...
                          if (neo4jresponse) {
                            return (
                              <div key={i}>
                                <Card className="p-4 my-3 bg-secondary/30 border-secondary/50 rounded-md">
                                  <pre>
                                    {JSON.stringify(neo4jresponse, null, 1)}
                                  </pre>
                                </Card>
                              </div>
                            );
